...
import exm.ast.Context.VisibleVariable;
...
import exm.ast.VariableUsageInfo.VInfo;
import exm.ast.descriptor.ArrayRange;
import exm.ast.descriptor.ForLoopDescriptor;
import exm.ast.descriptor.ForLoopDescriptor.LoopVar;
import exm.ast.descriptor.ArrayElems;
import exm.ast.descriptor.ForeachLoop;
import exm.ast.descriptor.FunctionDecl;
import exm.ast.descriptor.If;
import exm.ast.descriptor.IterateDescriptor;
import exm.ast.descriptor.LValue;
import exm.ast.descriptor.Switch;
import exm.ast.descriptor.Update;
import exm.ast.descriptor.VariableDeclaration;
import exm.ast.descriptor.Wait;
import exm.parser.CompilerBackend;
import exm.parser.Settings;
import exm.parser.antlr.ExMParser;
import exm.parser.gen.TurbineGenerator;
import exm.parser.ic.SwiftICGenerator;
import exm.parser.ic.ICInstructions.Oparg;
import exm.parser.util.*;
import exm.parser.util.FilePosition.LineMapping;
import exm.parser.util.TernaryLogic.Ternary;

public class SwiftScript {

  String inputFile;
  PrintStream output;
  PrintStream icOutput; // output stream for intermediate code
  ...
  public SwiftScript(String inputFile, String timestamp, PrintStream output,
      PrintStream icOutput, LineMapping lineMapping) {
    this.inputFile = inputFile;
    this.timestamp = timestamp;
    this.backend = new SwiftICGenerator(logger, icOutput);
    this.typecheck = new TypeChecker(logger);
    this.varAnalyzer = new VariableUsageAnalyzer(logger, typecheck,
                                                        lineMapping);
    // this.includePath = new IncludePath(logger);
    // setupIncludePath();
    // swap in this code generator for abstract machine code generation
    // generator = new TICGenerator();
    this.output = output;
  ...
  public void walk(SwiftAST tree) throws UserException {
    ...
    if (logger.isDebugEnabled())
      logger.debug(ParserUtils.printTree(tree));
 ...
 throws UserException {
  ...
  private String extractStringLit(Context context, SwiftAST tree) throws InvalidSyntaxException {
    ...
    String rawLiteral = tree.child(0).getText();
    String result = ParserUtils.unescapeString(context,
          ParserUtils.unquote(rawLiteral));
    logger.trace("Unescaped string '" + rawLiteral + "', resulting in '"
        + result + "'");
    return result;
  }
  ...
  throws UserException
  {
    ...
    String pkg       = tree.child(3).getText();
    String version   = tree.child(4).getText();
    String symbol    = tree.child(5).getText();

    pkg     = ParserUtils.unquote(pkg);
    version = ParserUtils.unquote(version);
    symbol  = ParserUtils.unquote(symbol);
